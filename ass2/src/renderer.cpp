...
const char *FRAG_PATH_2 = "res/shaders/normal_and_specular.frag";
...
renderer_t make_renderer(const glm::mat4 &projection) {
    ...
    GLint light_color_loc_1 = chicken3421::get_uniform_location(render_program, "u_light_color_1");
    GLint light_pos_loc_1 = chicken3421::get_uniform_location(render_program, "u_light_pos_1");
    GLint light_color_loc_2 = chicken3421::get_uniform_location(render_program, "u_light_color_2");
    GLint light_pos_loc_2 = chicken3421::get_uniform_location(render_program, "u_light_pos_2");
    ...
    light_pos_loc_1, light_color_loc_1,
    light_pos_loc_2, light_color_loc_2, diff_map_loc, spec_map_loc,
    diffuse_loc, emissive_loc, shininess_loc };
}

void draw(const renderer_t &renderer, const glm::mat4 &p, const glm::mat4 &v, const glm::mat4 &m, const model_t &model) {
    const auto &mtl = model.materials[0];
...
void render(const renderer_t &renderer, const scene_t &scene) {
    ...
    glm::mat4 v = get_view(scene.main_cam);


    // set up the lighting
    glUniform3fv(renderer.light_pos_loc_1, 1, glm::value_ptr(scene.point_light_1.pos));
    glUniform3fv(renderer.light_color_loc_1, 1, glm::value_ptr(scene.point_light_1.color));
    
    glUniform3fv(renderer.light_pos_loc_2, 1, glm::value_ptr(scene.point_light_2.pos));
    glUniform3fv(renderer.light_color_loc_2, 1, glm::value_ptr(scene.point_light_2.color));
    ...
    while (!renderables.empty()) {
        ...
        if (node.kind == node_t::LIGHT_SPHERE) {
